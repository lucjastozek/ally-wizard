...
export function displayNextSteps(
  ...
  if (hasCI && selectedTools.length > 0) {
    displayCISetupInstructions();
  }
...
/**
 * Displays CI setup instructions
 */
function displayCISetupInstructions() {
  logMessage("CI Setup Required:", MESSAGE_TYPES.SECTION);
  logMessage(
    "To enable automated testing in GitHub Actions:",
    MESSAGE_TYPES.INFO
  );
  logMessage("1. Go to your repository settings", MESSAGE_TYPES.INFO);
  logMessage(
    "2. Navigate to 'Secrets and variables' > 'Actions'",
    MESSAGE_TYPES.INFO
  );
  logMessage("3. Add a new secret: A11Y_GITHUB_TOKEN", MESSAGE_TYPES.INFO);
  logMessage("4. Use a GitHub token with repo permissions", MESSAGE_TYPES.INFO);
  logMessage(
    `Learn more: ${RESOURCE_LINKS.GITHUB_SECRETS}`,
    MESSAGE_TYPES.INFO
  );
}
